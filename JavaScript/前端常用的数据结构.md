...
1. 事件队列：在 JavaScript，存在异步操作的时候会将异步操作的结果放到一个队列里面，等待同代操作结束后，再从队列中依次取出结果来使用。
...
![](./images/前端常用的数据结构/2.png)

下面的图，就不是完全二叉树：

![](./images/前端常用的数据结构/3.png)

针对完全二叉树，有一个应用，就是最大堆和最小堆。

- 最大堆：父节点的数值大于或者等于所有的子节点的数值
- 最小堆：刚好相反，父节点的数值小于或者等于所有的子节点的数值

最大堆的示例

![](./images/前端常用的数据结构/4.png)

最小堆的示例

![](./images/前端常用的数据结构/5.png)

关于这种最大堆或者最小堆（完全二叉树），仍然是可以使用数组的方式来存储：

![](./images/前端常用的数据结构/6.png)
